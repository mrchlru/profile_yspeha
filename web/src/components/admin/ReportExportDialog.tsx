...
import type { ReportExportCandidate, ReportExportScope } from "@/lib/admin/reportExportKinds";
import {
  REPORT_EXPORT_FILE_FORMAT_LABELS,
  REPORT_EXPORT_TEST_KIND_LABELS,
  REPORT_EXPORT_TEST_KINDS,
  reportExportSupportsManagerVariant,
  type ReportExportFileFormat,
  type ReportExportTestKind,
  type ReportExportVariant,
...
export type ReportExportDialogProps = {
  open: boolean;
  onClose: () => void;
  /** Предвыбор папок из списка результатов (галочки на карточках). */
  initialFolderKeys?: ReadonlyArray<string>;
};

/**
 * Модальное окно выгрузки отчётов из раздела «Результаты тестирования».
 */
export function ReportExportDialog({
  open,
  onClose,
  initialFolderKeys = [],
}: ReportExportDialogProps): React.ReactElement | null {
  ...
  const [reportVariant, setReportVariant] = useState<ReportExportVariant>("full");
  const [fileFormat, setFileFormat] = useState<ReportExportFileFormat>("pdf");
  ...
  useEffect(() => {
    ...
    if (!reportExportSupportsManagerVariant(testKind)) {
  ...
  const loadCandidates = useCallback(async (kind: ReportExportTestKind): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/reports/export/candidates?${params.toString()}`, {
        cache: "no-store",
      });
      ...
      setCandidates([]);
      setError(_fetchErrorMessage(err, "Не удалось загрузить список людей."));
    } finally {
  ...
  const supportsManager = reportExportSupportsManagerVariant(testKind);
  ...
  async function handleDownload(): Promise<void> {
    ...
    try {
      ...
      const fileName = _fileNameFromDisposition(res.headers.get("Content-Disposition")) ?? "otchety.zip";
      ...
      setError(
        _fetchErrorMessage(
          err,
          "Не удалось скачать выгрузку. При режиме «все последние» попробуйте выбрать нескольких людей вручную — серверу нужно время на PDF."
        )
      );
    } finally {
      setBusy(false);
      ...
      >
        <div className="border-b border-black/5 px-5 py-4">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 id="report-export-title" className={adminPanelSectionTitleClass}>
                Выгрузка отчётов
              </h2>
              <p className={`mt-1 ${adminPanelMutedTextClass}`}>
                Файлы: «Фамилия_Имя_дата прохождения». Скрининг и ОД/ТУ — PDF и/или Word; выгорание и ПРОФ СБ — Word или HTML вместо PDF.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <fieldset className="space-y-2">
            <legend className={stepLabelClass}>Что выгрузить</legend>
            <label className="flex cursor-pointer items-center gap-2 text-[14px]">
              ...
              />
              Все последние прохождения выбранного типа теста
            </label>
            <label className="flex cursor-pointer items-center gap-2 text-[14px]">
              <input
                type="radio"
          ...
          <div>
            <label htmlFor="export-test-kind" className={`block ${stepLabelClass}`}>
              Тип теста
            </label>
            <select
              id="export-test-kind"
          ...
          {supportsManager ? (
            <fieldset className="space-y-2">
              <legend className={stepLabelClass}>Вариант отчёта</legend>
              <label className="flex cursor-pointer items-center gap-2 text-[14px]">
                <input
                  type="radio"
                  name="export-variant"
                  checked={reportVariant === "full"}
                  onChange={() => setReportVariant("full")}
                />
                Полный отчёт (HrD)
              </label>
              <label className="flex cursor-pointer items-center gap-2 text-[14px]">
                <input
                  type="radio"
                  name="export-variant"
                  checked={reportVariant === "manager"}
                  onChange={() => setReportVariant("manager")}
                />
          ...
          <fieldset className="space-y-2">
            <legend className={stepLabelClass}>Формат файла</legend>
            {(Object.keys(REPORT_EXPORT_FILE_FORMAT_LABELS) as ReportExportFileFormat[]).map(
              (format) => (
                <label key={format} className="flex cursor-pointer items-center gap-2 text-[14px]">
                  ...
                  />
                  {REPORT_EXPORT_FILE_FORMAT_LABELS[format]}
                </label>
              )
            )}
          </fieldset>
            ...
            <div className="space-y-3">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <label htmlFor="export-person-search" className={`block ${stepLabelClass}`}>
                    Люди
                  </label>
                  <input
                    id="export-person-search"
                    type="search"
                    value={personQuery}
              ...
              {initialFolderKeys.length > 0 ? (
                <p className={`text-[13px] ${adminPanelMutedTextClass}`}>
                  Учтены {String(initialFolderKeys.length)} папок, отмеченных в списке результатов.
                </p>
              ) : null}
...
function _fetchErrorMessage(err: unknown, fallback: string): string {
  if (!(err instanceof Error)) {
    return fallback;
  }
  const msg = err.message.toLowerCase();
  if (msg.includes("failed to fetch") || msg.includes("networkerror") || msg.includes("load failed")) {
    return `${fallback} (браузер не получил ответ от сервера — часто из‑за долгой генерации или обрыва соединения, а не из‑за Wi‑Fi).`;
  }
  return err.message.length > 0 && err.message.length < 200 ? err.message : fallback;
}
