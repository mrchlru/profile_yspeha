...
import { burnoutSyncAnswersBodySchema } from "@/lib/validation/burnoutSyncAnswersSchema";
...
export async function POST(
  ...
  const parsed = burnoutSyncAnswersBodySchema.safeParse(jsonBody);
  if (!parsed.success) {
    screeningServerLog("burnout_sync", "validation_failed", {
  ...
  const existing = await prisma.burnoutSubmission.findUnique({
  ...
  try {
    ...
    screeningServerLog("burnout_sync", "answers_saved", { sessionRef });
  } catch (err) {
    screeningServerLog("burnout_sync", "save_failed", {
      sessionRef,
